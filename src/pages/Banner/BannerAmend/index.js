...
class BannerAmend extends Component{
    ...
    async componentDidMount(){
        ...
        this.setState({banner_id:list.banner_id,banner_name:list.banner_name,banner_type:list.banner_type})
    }
    //修改商品
    submit= async ()=>{
        let {id} = this.props.match.params
        //调用修改的接口
        const {banner_id, banner_name, banner_type } = this.state
        let result = await bannerApi.bannerupdata({_id: id, banner_id, banner_name, banner_type })
        console.log(result)
    ...
    render(h) {
        let {banner_id, banner_name ,banner_type} = this.state
        return(
            <div>
                <Card title='广告修改'>
                    <span>广告ID:</span><br/><Input placeholder={banner_id}  style={{width:'360px'}} onChange={(e)=>{
                        // console.log(e)
                        this.setState({banner_id:e.target.value})
